...
- **Google OAuth consent screen — branding done, domain line NOT fixable without a custom domain (2026-09-07):** ✅ The "Sign in with Google" dialog said *"to continue to xbvnzfyheimezuikhpnz.supabase.co"*. **Root cause: Google renders the root domain of the OAuth CALLBACK, not the app name.** Our callback is Supabase's (`…supabase.co/auth/v1/callback`), and Google passes it through the chooser URL as `app_domain=https://xbvnzfyheimezuikhpnz.supabase.co`. Nothing in `AuthForm.tsx` touches this — its `redirectTo` is the hop *after* Supabase. **What was done** on Cloud project `rotpitch` (owner account `sahni.anand5.7@gmail.com`): app name `rotpitch` → **`RotPitch`**; app-domain links set to `https://rotpitch.com` + `/privacy` + `/terms`; `rotpitch.com` **verified in Google Search Console** via the HTML-file method (`apps/web/public/googlea3a82785038a33ca.html` — **never delete it**, verification is lost); publishing status flipped **Testing → In production**. **Measured result: the domain line did NOT change** — only the Privacy/Terms links appeared, still attributed to the supabase.co host. Re-tested after publishing: unchanged. **So the ONLY remaining fix is a Supabase custom domain** (paid add-on + Pro plan) so the callback becomes e.g. `auth.rotpitch.com` — note **not** `api.rotpitch.com`, already taken by the Railway API. **Two incidental findings:** (1) `rotpitch.com` and `xbvnzfyheimezuikhpnz.supabase.co` were *already* both Authorized Domains — Google accepts a supabase.co subdomain you don't own, so that was never the blocker; (2) the app had been sitting in **Testing with 0 test users**, meaning real visitors clicking "Continue with Google" were blocked — publishing fixed a live signup bug that was unrelated to the branding. **The logo was deliberately NOT uploaded:** a logo forces a Google app-verification review on publish, while no-logo + non-sensitive scopes (`email`, `profile`) publishes with no review. Upload it only when ready to sit through verification — ideally once, together with the custom domain. `layout.tsx` also supports the meta-tag verification method via `GOOGLE_SITE_VERIFICATION` (unused; the committed file was preferred since Google requires the proof to persist and an env var can be dropped).
