...
> Status: **IN BUILD** — Phases 1 (foundation, DB, shared, design system, full auth, all app screens), 4 + 5 (render pipeline) are done; render verified end-to-end (upload → FFmpeg composite → Supabase output → done, 1 credit deducted/refunded). Background loops live in the Supabase `backgrounds` bucket named `{style_key}.mp4` (picker preview + worker read the same file). Upload validation now enforced in 3 layers (client type/size/≤60s · `raw-uploads` bucket MIME+50 MB cap · worker `ffprobe` re-check → `videos.failure_reason` shown on failed cards). Owner gameplay asset pack uploaded. **Auto-captions are live** (Phase 6, part 1): worker transcribes the demo audio via OpenAI Whisper and burns styled captions in with libass (`subtitles` filter); verified end-to-end through the real render path (live Whisper HTTP call pending an `OPENAI_API_KEY` in `.env`). Remaining/deferred: per-job progress % (cosmetic at ~8 s renders), S3 output for prod (Phase 9 swap at `uploadFrom`/`OUTPUT_BUCKET`); not started: Phase 6 voiceover (ElevenLabs), 7 (Stripe + Razorpay billing), 9 (deploy). See `TASKS.md` for granular status.
...
- **Deploy config (Phase 9):** ✅ **Railway = one Dockerfile (`apps/api/Dockerfile`), two services** — `api` (default CMD `start`, `/health` check, `$PORT`) and `worker` (CMD override `pnpm --filter @rotpitch/api start:worker`, no HTTP). Image = `node:20-bookworm-slim` + apt `ffmpeg` (ships libass/freetype/fontconfig) + `fonts-liberation` (caption fallback font — resolves the Phase 6 `fontsdir` gap for now) + pnpm workspace install; **build context is the repo root**. **Vercel = `apps/web`** root dir, Next.js preset (`transpilePackages` handles `@rotpitch/shared`). **Redis = Upstash** (`rediss://` connect URL, not REST). Outputs stay on **Supabase** (S3 still deferred). Root `.dockerignore` keeps `.env*` out of the image. Full runbook in **`DEPLOY.md`**.
